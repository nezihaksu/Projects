...
class airbnb_istanbul:
	...
	def preprocess(self):
		...
		useless = ['id', 'listing_url', 'scrape_id', 'last_scraped', 'thumbnail_url', 'medium_url', 'picture_url',
                    ...
                    'number_of_reviews_ltm', 'requires_license', 'license', 'jurisdiction_names', 'neighbourhood_group_cleansed', 'host_total_listings_count', 'square_feet', 'weekly_price', 'monthly_price', 'security_deposit', 'zipcode', 'neighbourhood_cleansed']
	...
	def to_float(self):
		...
		self.df['extra_people'] = self.df['extra_people'].apply(
			lambda s: float(s[1:].replace(',', '')))
		...
		self.df = self.df[self.df.cleaning_fee != 0]
	...
	def drop_na_row(self):
		...
		self.df['cleaning_fee'] = self.df['cleaning_fee'].map(
			lambda s: s.lstrip('$').rstrip('$'))
		self.df['cleaning_fee'] = self.df['cleaning_fee'].apply(
			lambda s: float(s[1:].replace(',', '')))
	...
	def encode_categorical(self):
		...
		self.df[non_float_labels] = self.df[non_float_labels].apply(
	...
	def encoding(self):
		...
		non_float_indexes = np.where(self.df.dtypes != 'float64')
		non_float_labels = self.df.columns[non_float_indexes[0]]
		self.df = pd.get_dummies(self.df, columns=non_float_labels)
		#self.df = self.df.join(y)
		#y = np.log(self.df['price'])
		pd.set_option('display.max_columns', None)
		#pd.set_option('display.max_rows',None)
		#self.df = self.df.drop('cleaning_fee',axis = 1)
	...
	def polytrans(self):
		...
		self.df['cleanliness_rating'] = None
		self.df[['review_scores_rating', 'review_scores_cleanliness', 'cleanliness_rating']
          ] = poly_transformer.fit_transform(self.df[['review_scores_rating', 'review_scores_cleanliness']])
		self.df = self.df.join(y)
		y = np.log(self.df['price'])
